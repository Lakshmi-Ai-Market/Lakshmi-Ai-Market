...
# 🔧 Function to fetch candles consistently
def fetch_dhan_candles(symbol, interval, limit):
    candles = fetch_candle_data(symbol)
    if not candles:
        print(f"⚠️ No candle data found for symbol: {symbol}")
        return []
    return candles[-limit:]  # Take latest 'limit' candles
...
def strategy_trend_bias(symbol):
    data = fetch_dhan_candles(symbol, "5m", 20)
...
def analyze_all_strategies(symbol="BANKNIFTY"):
    strategies = [
        strategy_trend_bias,
        strategy_breakout,
        strategy_momentum,
